...
- Each `Stop` payload contained a response equal to the response reconstructed
  from the provider transcript. The v3 adapter therefore uses the completion
...
The API v3 durable adapter therefore reports Claude Code as unsupported; only the legacy Pane
