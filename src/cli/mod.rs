...
use agent::{AgentCommand, read_prompt_input};
use api::ApiCommand;
use pane::PaneCommand;
...
pub fn run() -> ExitCode {
    ...
    let input = match agent_hook_deadline {
        ...
        None if agent::body_requires_stdin(&args) => match read_prompt_input(std::io::stdin()) {
            Ok(input) => input,
            Err(error) => {
    ...
    W: Write,
{
    ...
    match cli.command {
        Command::Api { command, json: _ } => {
            Ok(Some(api::dispatch(command, runner, env, now_epoch)?))
        }
        Command::Pane { command, json: _ } => {
            Ok(Some(pane::dispatch(command, runner, env, now_epoch)?))
        }
        Command::Agent { command, json: _ } => Ok(Some(agent::dispatch(
            command, input, runner, env, now_epoch,
        )?)),
