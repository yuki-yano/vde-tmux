...
- [x] `vt statusline-summary` が `#[fg=red]▲2#[default] #[fg=green]●1#[default]` 形式(0件の状態は省略、agent 0体なら空文字列)を出力する
- [x] グリフは `badge.glyphs` を使用。色の既定は blocked=red / working=green / done=cyan / idle=色なし
- [x] daemon 稼働時は unread を反映した正確なカウント(✓ と ○ を区別)、daemon 停止時は list-panes フォールバック(✓ は ○ に縮退)で動作する
- [x] `statusline.summary.enabled: false` で空文字列を返す(旧 `agent_badge.enabled` の dead config が解消される)
- [x] `vt statusline-agent-badge` は存在しない(コマンド・protocol とも削除)

### テスト完了条件

- [x] `rtk cargo test` 全通過
- [x] 新規テスト: カウント集計(unread 込み)、0件省略、agent 0体で空、enabled=false で空、フォールバック経路、protocol roundtrip
- [x] `rtk cargo clippy --all-targets` 警告ゼロ、`rtk cargo fmt --check` 通過

### 運用反映条件

- [x] `docs/e2e-smoke.md` の `running:1` 期待を summary 形式に更新し、smoke 実施を記録
- [x] `docs/migration.md` に `statusline-agent-badge` → `statusline-summary` の書き換え(M7 の dotfiles 変更項目)と config キー変更(`statusline.agent_badge` → `statusline.summary`)を追記
- [x] `docs/statusline-ui-proposals.md` §7.2 Step 2 にチェック
...
- 計画からの逸脱なし。
- daemon 経路は unread を反映して done(✓)と idle(○)を区別し、fallback 経路は unread 履歴を持たないため done を idle 側へ縮退する設計どおりに実装した。
