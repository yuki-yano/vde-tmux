...
while True:
    ...
    s.settimeout(5)
    s.connect(path)
    s.sendall(b'{"op":"hello","proto":25}\n')
    reader = s.makefile("rb")
    hello = json.loads(reader.readline())
    assert hello["type"] == "hello_ack" and hello["proto"] == 25, hello
    if hello["phase"] == "serving":
        break
    s.close()
    assert time.time() < deadline, hello
    time.sleep(0.1)
s.sendall(b'{"op":"query_resolved_snapshot","proto":25}\n')
reply = json.loads(reader.readline())
assert reply["type"] == "resolved_snapshot_result", reply
assert reply["snapshot"]["panes"] == [], reply
print("empty-topology protocol v25 Serving ok")
PY
...
for path in sys.argv[1:]:
    ...
    while True:
        ...
        client.settimeout(5)
        client.connect(path)
        client.sendall(b'{"op":"hello","proto":25}\n')
...
wait_for_topology() {
  for _ in $(seq 1 80); do
    query_v25 '{"op":"query_resolved_snapshot","proto":25}'
...
wait_for_topology
echo "protocol v25 linked-window topology converged"
...
def query(context):
    ...
    request = {"op":"query_status_snapshot", "proto":25, "context":context}
...
while True:
    topology = request({"op":"query_resolved_snapshot","proto":25})
...
deadline = time.time() + 5
while True:
    reply = request({"op":"query_resolved_snapshot","proto":25})
...
after_first = request({"op":"query_resolved_snapshot","proto":25})
...
wait_for_restart_category_projection() {
  ...
  while (( SECONDS < deadline )); do
    query_v25 '{"op":"query_status_snapshot","proto":25,"context":"global"}'
...
client.sendall(b'{"op":"hello","proto":25}\n')
assert json.loads(reader.readline())["type"] == "hello_ack"
client.sendall(b'{"op":"query_resolved_snapshot","proto":25}\n')
reply = json.loads(reader.readline())
pane = {"pane_id":pane_id,"pane_pid":int(pane_pid)}
...
snapshot = request({"op":"query_resolved_snapshot", "proto":25})["snapshot"]
