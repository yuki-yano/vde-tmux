# Plan 26: Sidebar ヘッダーのリッチ化(案E: format 対応 powerline バッジ + フィルタチップ)
...
- ヘッダーを **案E**(1行目: format 対応 powerline モードバッジ + 総数セグメント、2行目: フィルタチップ)の2段構成にする
...
- **1行目**
  - モードバッジ ` ≣ category `: `sidebar.header.format` の `{label}` を `≣ category` 形式の固定幅 label に置換して描画する。既定は `format: " {label} "` / `suffix: ""` / `bold: true` / `colors.fg: "16"` / `colors.bg: "147"` / `colors.outer_bg: "235"`。クリックで `CycleViewMode`(既存)。`view_mode_label_padded` の固定幅を維持する。
  - powerline 矢印 ``(U+E0B0): `sidebar.header.suffix` で表現する。`suffix: ""` なら矢印なし。suffix の fg はモードバッジ bg、bg は `colors.outer_bg`(未指定なら総数セグメント bg)。
...
- `sidebar.header` config は statusline の segment と同じ考え方で `format` / `prefix` / `suffix` / `bold` / `colors` を持つ。`powerline` / `separator` は受け付けない(後方互換は取らない)。powerline 矢印を消す場合は `suffix: ""` を設定する。
...
- Modify: `src/config/schema.rs` / `src/config/mod.rs`(`header.format/prefix/suffix/bold/colors` 採用、`header.powerline/separator` 廃止)
...
3. theme に反転用 fg(例: `header_badge_fg`、default は端末黒相当の `Color::Indexed(16)`)と `header_format` / `header_prefix` / `header_suffix` / `header_outer_bg` を追加。`SidebarRenderTheme` の色規約コメントも更新。
4. config: `sidebar.header` は `format/prefix/suffix/bold/colors` を持つ statusline 風 segment 設定にする。`powerline` / `separator` は受け付けない。schema テスト(253-262)を更新。
...
1. README: 2段ヘッダーの表示仕様、チップの状態表現(アクティブ反転 / 0件 dim・非活性)、`sidebar.header` の `format/prefix/suffix/bold/colors` と `suffix: ""` による powerline 無効化を明記。
...
```bash
rtk git add -A
rtk git commit -m "sidebar ヘッダー設定を statusline 風 format に寄せる"
```
...
- [x] ヘッダーが2行(1行目: format 対応 powerline モードバッジ + 総数セグメント、2行目: フィルタチップ)で描画される
- [x] バッジ件数がフィルタ適用前の全 panes(TRIAGE 含む)から算出され、フィルタ適用中も他状態の件数が変わらない
- [x] アクティブチップは状態色反転 + bold(0件化しても `▲ 0` で反転維持)、非アクティブ非0チップは `active_bg` + 状態色、0件チップは dim・クリック不可
- [x] 0件フィルタはクリックでも `tab` サイクルでも適用されない(`all` は常に適用可、全状態0件時のサイクルは `all` に収束)
- [x] 適用中に0件化してもフィルタは自動解除されず、ヘッダー表示とチップ操作が維持され、空状態に `no {name} agents` + 復帰ヒントが表示される
- [x] `all` フィルタで0件のときは従来どおり `no agents` のみ表示される
- [x] 幅不足時は総数セグメント省略 → truncate の順で退避し、モード切替でチップ位置がずれない
- [x] `sidebar.header.suffix: ""` で矢印グリフなしの矩形塗りにフォールバックする

### テスト完了条件

- [x] 新規テスト: counts の全体算出(フィルタ・TRIAGE 込み)、`SetFilter` の0件ガード、`ToggleFilter` の0件スキップと `all` 収束、2行レイアウトのテキストとヒットテスト(0件チップ・総数セグメントの非活性含む)、チップの Style 3態、`suffix: ""`、幅フォールバック、空状態でのヘッダー維持とヒント表示、rows 空でのチップクリック
- [x] `rtk cargo test` 全通過(既存ヘッダーテスト・config schema テストの期待値更新を含む)
- [x] `rtk cargo clippy --all-targets` 警告ゼロ、`rtk cargo fmt --check` 通過

### 運用反映条件

- [x] README にヘッダー表示仕様と `sidebar.header` config の変更(`format/prefix/suffix/bold/colors`・`suffix: ""` による powerline 無効化)を明記
- [x] `docs/e2e-smoke.md` に確認手順を追記し、実機 smoke を実施
- [x] `docs/sidebar-header-proposals.html` の状態1〜3と実装の表示が一致することを確認
...
- powerline 矢印 ``(U+E0B0)は nerd font 前提。`sidebar.header.suffix: ""` で無効化できるようにする。
