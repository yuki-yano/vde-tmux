...
- [x] pin 中の chat 行のマーカーが `✦`(既定ラベンダー、`colors.pin`)になり、pin 中の meta 1行にも `✦ ` が付く
- [x] ヘッダーの mode が `≣ {mode}`(既定ラベンダー、`colors.header_mode`)、category 行が `◆ {name}`(既定ピーチ、`colors.category`)、repo 行が青のままとなり、3役が色+グリフで区別できる
- [x] ByCategory 表示で category 行の残余幅が `─` の罫線で埋まり、グループ境界が線で読める
- [x] LIVE が幅 24 列以上で `╭╴LIVE …╮ │…│ ╰─╯` の角丸ボーダーカードになる(枠は marker 色、`LIVE` ラベルは live 色のまま)。幅 24 未満は従来表示
- [x] fisheye フル展開の末尾行が `↗ jump   ⌕ preview` の 1 行 2 ボタンになり、それぞれのクリックでジャンプ / popup プレビューが即時実行される
- [x] detail 行(グレー文字)のクリックが「親 chat の手動展開トグル」になり、preview は開かない。ダブルクリック判定(250ms 待ち)が全廃され、chat 行クリック(pin)も即時反応する
- [x] ユーザーが見ている window の agent(`window_active && session_attached`)の chat 行に薄背景(`colors.active_bg`)、その系譜(category / repo / chat / detail / jump)に左端バー `▎`(`colors.active_bar`)が付く。カーソル行では選択ハイライトが優先される

### テスト完了条件

- [x] `rtk cargo test` 全通過(既存テストの期待値更新を含む)
- [x] 新規テスト: pin グリフ描画、row_style の Category/Repo 分離、mode セグメントのグリフとスタイル、category 罫線、LIVE ボーダー(広幅/狭幅)、compute_areas のボーダー分確保、jump 行の列 hit-test、detail クリックの展開トグル(runtime)、active 導出と祖先伝播、active 行の bar/bg 描画、新 config キーのパース
- [x] `rtk cargo clippy --all-targets` 警告ゼロ、`rtk cargo fmt --check` 通過

### 運用反映条件

- [x] `docs/e2e-smoke.md` に 6 項目の確認手順と smoke 実施結果を追記
- [x] README に新 config キー(`colors.pin` / `colors.category` / `colors.header_mode` / `colors.active_bg` / `colors.active_bar`)の説明を追記
- [x] `docs/sidebar-improvements-proposals.html` に対応する項目が全て実装済みであることを本計画書のチェックで担保(HTML 自体の更新は不要)
...
- Task 1 実装中の追加要望により、ヘッダーの状態アイコンと数字の間にスペースを入れた(`≡ 0 ▲ 0 ● 0 ✓ 0 ○ 0`)。hit-test は表示幅に追従するよう既存 range 計算の期待値を更新した。
- Task 3 の LIVE カード化は既存の ANSI 色保持(`ansi-to-tui`)と入力欄 cut marker を維持したまま、幅 24 列以上のみ枠を追加した。
- Task 4 の runtime 仕様メモを近接コードにコメントとして残した。detail/meta クリックは preview ではなく親 chat の手動展開固定/解除として扱い、キーボード `Enter` / `p` の preview 動作は変更していない。
- Task 5 の active 判定は計画どおり `window_active && session_attached` を採用した。複数 client が別 window を見ている場合は複数系譜に active が付く仕様としてコメント化した。
- TUI の alt-screen capture は既存 smoke と同様に安定しないため、Plan 23 smoke は `cargo install --path . --force` のバイナリ反映と render/tui/runtime/tree の回帰テストで各表示・クリック仕様を確認した。
