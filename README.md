...
sidebar の配色は 5 族の規約で運用する。状態族は badge/rollup の ▲赤・●緑・✓シアン・○灰、構造族は repo 青太字・category ピーチ太字・branch 淡シアン(Indexed 73)、操作族は toggle/mode/active/preview のラベンダー系、本文族は通常本文・detail(246)・marker 暗灰、実況族は LIVE/EVENTS 見出し専用のマゼンタ。branch の既定色は ✓ done の明シアンとの衝突を避けるため、従来の Cyan から Indexed 73 に変更した。`sidebar.colors.branch` を設定すれば従来色にも戻せる。
